...
{
	INT_Disable();
	//LEUART0->IFC = LEUART_IFC_RX;
/*	int temp_tens,temp_units;
	if(count == 0)
	{
		temp_tens = LEUART0->RXDATA;
		temp_tens -= 48;
		count = 1;
	}
	else
	{
		temp_units = LEUART0->RXDATA;
		temp_units -=48;
		count = 0;
	}*/
	//temperature = temp_tens*10 + temp_units;
	temperature = LEUART0->RXDATA;
	INT_Enable();
}
